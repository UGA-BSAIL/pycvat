...
class PasswordChange(object):
    ...
    def old_password(self, old_password):
        ...
        if (
            ...
            and len(old_password) > 128
        ):
            raise ValueError(
                "Invalid value for `old_password`, length must be less than or equal to `128`"
            )  # noqa: E501
        if (
            self._configuration.client_side_validation
            and old_password is not None
            and len(old_password) < 1
        ):
            raise ValueError(
                "Invalid value for `old_password`, length must be greater than or equal to `1`"
            )  # noqa: E501
