    ...
    try:
        ...
        if rq_job is not None:
            if rq_job.is_queued or rq_job.is_started:
                ...
                return JsonResponse(
                    {"status": rq_job.get_status(), "stderr": rq_job.exc_info}
                )
        else:
            return JsonResponse({"status": "unknown"})
    except Exception as ex:
        slogger.glob.error(
            "error occured during checking repository request with rq id {}".format(
                rq_id
            ),
            exc_info=True,
        )
...
def create(request, tid):
    try:
        ...
        slogger.glob.error(
            "error occured during initial cloning repository request with rq id {}".format(
                rq_id
            ),
            exc_info=True,
        )
...
def push_repository(request, tid):
    try:
        ...
        queue.enqueue_call(
            func=CVATGit.push,
            args=(tid, request.user, request.scheme, request.get_host()),
            job_id=rq_id,
        )
        ...
        try:
            slogger.task[tid].error(
                "error occured during pushing repository request",
                exc_info=True,
            )
        except Exception:
            pass
...
def get_repository(request, tid):
    try:
        ...
        try:
            slogger.task[tid].error(
                "error occured during getting repository info request",
                exc_info=True,
            )
        except Exception:
            pass
...
def get_meta_info(request):
    try:
        ...
        for db_git in db_git_records:
            response[db_git.id] = db_git.status

        return JsonResponse(response, safe=False)
    except Exception as ex:
        slogger.glob.exception(
            "error occured during get meta request", exc_info=True
        )
        return HttpResponseBadRequest(str(ex))
